...
if [ "$1" = "gcc" ]; then
    ...
    if [ "$2" = "debug" ]; then
        $EMCC $CXXFLAGS -ferror-limit=2 -O0 -g4 platform_emscripten.cpp -o obst.js -s "BINARYEN_METHOD='native-wasm'" -s EXPORTED_FUNCTIONS="[$exp_fun]" -s DEMANGLE_SUPPORT=1 -s SAFE_HEAP=1 -s ASSERTIONS=2 -s STACK_OVERFLOW_CHECK=2 --source-map-base "file://`pwd`/"
    elif [ "$2" = "release" ]; then
